        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalUserRoles = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalUserRoles">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add UserRole</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="UserRoleId" type="text" placeholder="Enter UserRoleId" v-model="userRoleToAdd.userRoleId"></base-input>
  <base-input label="UserProfileId" type="text" placeholder="Enter UserProfileId" v-model="userRoleToAdd.userProfileId"></base-input>
  <base-input label="RoleName" type="text" placeholder="Enter RoleName" v-model="userRoleToAdd.roleName"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="userRoles" :row-key="record => record.UserRoleId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <UserRolePictureView :userRoles="userRoles" />
          </div>
...
import UserRolePictureView from './UserRolePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'userRoleId',
          visible: true,
          scopedSlots: { customRender: 'userRoleId' },
          sorter: true
          //sorter: (a, b) => a.userRoleId - b.userRoleId,
          //sorter: (a, b) => a.userRoleId.localeCompare(b.userRoleId),
        },
        {
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} userRoles`,
      },
      ...
      sortBy: 'userRoleId',           // Column to sort by
  ...
  methods: {
    ...
    async renderUserRolesTable() {
      ...
      let userRolesTableData = [];
      for (let i = 0; i < this.userRoles.length; i++) {
        userRolesTableData.push({
          id: i,
          userRoleId: this.userRoles[i].userRoleId,
          year: this.userRoles[i].year,
          date: this.userRoles[i].date,
          competitionId: this.userRoles[i].competitionId,
          userRoleId: this.userRoles[i].userRoleId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-user-roles',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForUserRolesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalUserRoles = false;

      const currentDate = new Date().getTime();
      this.userRoleToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.userRoleToAdd);
      console.log(jsonData);
      
      const res = await UserRoleService.addUserRole(jsonData);
