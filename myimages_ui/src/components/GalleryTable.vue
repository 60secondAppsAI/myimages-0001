        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Gallerys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalGallerys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalGallerys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Gallery</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="GalleryId" type="text" placeholder="Enter GalleryId" v-model="galleryToAdd.galleryId"></base-input>
  <base-input label="OwnerId" type="text" placeholder="Enter OwnerId" v-model="galleryToAdd.ownerId"></base-input>
  <base-input label="GalleryName" type="text" placeholder="Enter GalleryName" v-model="galleryToAdd.galleryName"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="gallerys" :row-key="record => record.GalleryId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <GalleryPictureView :gallerys="gallerys" />
          </div>
...
import GalleryPictureView from './GalleryPictureView.vue';


const gallerysColumns = [
  "galleryId",
  "year",
  "date",
  "competitionId",
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'galleryName',
          visible: true,
          scopedSlots: { customRender: 'galleryName' },
          sorter: true
          //sorter: (a, b) => a.galleryName - b.galleryName,
          //sorter: (a, b) => a.galleryName.localeCompare(b.galleryName),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} gallerys`,
      },
      ...
      gallerysTable: {
        columns: [...gallerysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'galleryId',           // Column to sort by
  ...
  methods: {
    ...
    async renderGallerysTable() {
      ...
      let gallerysTableData = [];
      for (let i = 0; i < this.gallerys.length; i++) {
        gallerysTableData.push({
          id: i,
          galleryId: this.gallerys[i].galleryId,
          year: this.gallerys[i].year,
          date: this.gallerys[i].date,
          competitionId: this.gallerys[i].competitionId,
          galleryId: this.gallerys[i].galleryId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-gallerys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForGallerysChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalGallerys = false;

      const currentDate = new Date().getTime();
      this.galleryToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.galleryToAdd);
      console.log(jsonData);
      
      const res = await GalleryService.addGallery(jsonData);
