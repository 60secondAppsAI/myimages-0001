        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalOrganizations = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalOrganizations">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Organization</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="OrganizationId" type="text" placeholder="Enter OrganizationId" v-model="organizationToAdd.organizationId"></base-input>
  <base-input label="OrganizationName" type="text" placeholder="Enter OrganizationName" v-model="organizationToAdd.organizationName"></base-input>
  <base-input label="ContactInformation" type="text" placeholder="Enter ContactInformation" v-model="organizationToAdd.contactInformation"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="organizations" :row-key="record => record.OrganizationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <OrganizationPictureView :organizations="organizations" />
          </div>
...
import OrganizationPictureView from './OrganizationPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    OrganizationPictureView
  },

  data() {
    return {
      modalOrganizations: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'organizationId',
          ...
          //sorter: (a, b) => a.organizationId.localeCompare(b.organizationId),
        },
        {
		...
		dataIndex: 'organizationName',
          ...
          sorter: true
          //sorter: (a, b) => a.organizationName - b.organizationName,
      ...
      sortBy: 'organizationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderOrganizationsTable() {
      ...
      let organizationsTableData = [];
      for (let i = 0; i < this.organizations.length; i++) {
        organizationsTableData.push({
          id: i,
          organizationId: this.organizations[i].organizationId,
          year: this.organizations[i].year,
          date: this.organizations[i].date,
          competitionId: this.organizations[i].competitionId,
          organizationId: this.organizations[i].organizationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-organizations',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForOrganizationsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalOrganizations = false;

      const currentDate = new Date().getTime();
      this.organizationToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.organizationToAdd);
      console.log(jsonData);
      
      const res = await OrganizationService.addOrganization(jsonData);
