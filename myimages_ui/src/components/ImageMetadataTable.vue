        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Image Metadatas</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalImageMetadatas = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalImageMetadatas">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ImageMetadata</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ImageMetadataId" type="text" placeholder="Enter ImageMetadataId" v-model="imageMetadataToAdd.imageMetadataId"></base-input>
  <base-input label="ImageId" type="text" placeholder="Enter ImageId" v-model="imageMetadataToAdd.imageId"></base-input>
  <base-input label="Resolution" type="text" placeholder="Enter Resolution" v-model="imageMetadataToAdd.resolution"></base-input>
  <base-input label="Format" type="text" placeholder="Enter Format" v-model="imageMetadataToAdd.format"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="imageMetadatas" :row-key="record => record.ImageMetadataId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ImageMetadataPictureView :imageMetadatas="imageMetadatas" />
          </div>
...
import ImageMetadataPictureView from './ImageMetadataPictureView.vue';


const imageMetadatasColumns = [
  "imageMetadataId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    ImageMetadataPictureView
  },

  data() {
    return {
      modalImageMetadatas: false,
        isTableView: true,

      columns: [
        {
          title: 'Image Metadata Id',
		dataIndex: 'imageMetadataId',
          visible: true,
          scopedSlots: { customRender: 'imageMetadataId' },
          sorter: true
          //sorter: (a, b) => a.imageMetadataId - b.imageMetadataId,
          //sorter: (a, b) => a.imageMetadataId.localeCompare(b.imageMetadataId),
        },
        {
          title: 'Image Id',
		dataIndex: 'imageId',
		...
		dataIndex: 'resolution',
          ...
          sorter: true
          //sorter: (a, b) => a.resolution - b.resolution,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} imageMetadatas`,
      },
      ...
      imageMetadatasTable: {
        columns: [...imageMetadatasColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'imageMetadataId',           // Column to sort by
  ...
  methods: {
    ...
    async renderImageMetadatasTable() {
      ...
      let imageMetadatasTableData = [];
      for (let i = 0; i < this.imageMetadatas.length; i++) {
        imageMetadatasTableData.push({
          id: i,
          imageMetadataId: this.imageMetadatas[i].imageMetadataId,
          year: this.imageMetadatas[i].year,
          date: this.imageMetadatas[i].date,
          competitionId: this.imageMetadatas[i].competitionId,
          imageMetadataId: this.imageMetadatas[i].imageMetadataId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-image-metadatas',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForImageMetadatasChanged', this.searchQuery);
		//this.renderImageMetadatasTable();
    },
	...
	async handleAddSubmitted() {
      this.modalImageMetadatas = false;

      const currentDate = new Date().getTime();
      this.imageMetadataToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.imageMetadataToAdd);
      console.log(jsonData);
      
      const res = await ImageMetadataService.addImageMetadata(jsonData);
