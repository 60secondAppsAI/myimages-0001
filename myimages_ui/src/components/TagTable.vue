        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalTags = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalTags">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Tag</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="TagId" type="text" placeholder="Enter TagId" v-model="tagToAdd.tagId"></base-input>
  <base-input label="TagName" type="text" placeholder="Enter TagName" v-model="tagToAdd.tagName"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="tags" :row-key="record => record.TagId" :pagination="pagination"
...
import TagPictureView from './TagPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'tagName',
          ...
          sorter: true
          //sorter: (a, b) => a.tagName - b.tagName,
      ...
      sortBy: 'tagId',           // Column to sort by
  ...
  methods: {
    ...
    async renderTagsTable() {
      ...
      for (let i = 0; i < this.tags.length; i++) {
        tagsTableData.push({
          ...
          year: this.tags[i].year,
          date: this.tags[i].date,
          competitionId: this.tags[i].competitionId,
          tagId: this.tags[i].tagId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-tags',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForTagsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.tagToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.tagToAdd);
      console.log(jsonData);
      
      const res = await TagService.addTag(jsonData);
