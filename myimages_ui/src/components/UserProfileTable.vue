        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalUserProfiles = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalUserProfiles">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add UserProfile</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="UserProfileId" type="text" placeholder="Enter UserProfileId" v-model="userProfileToAdd.userProfileId"></base-input>
  <base-input label="UserName" type="text" placeholder="Enter UserName" v-model="userProfileToAdd.userName"></base-input>
  <base-input label="UserEmail" type="text" placeholder="Enter UserEmail" v-model="userProfileToAdd.userEmail"></base-input>
  <base-input label="CreatedDate" type="text" placeholder="Enter CreatedDate" v-model="userProfileToAdd.createdDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="userProfiles" :row-key="record => record.UserProfileId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <UserProfilePictureView :userProfiles="userProfiles" />
          </div>
...
import UserProfilePictureView from './UserProfilePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} userProfiles`,
      },
      ...
      sortBy: 'userProfileId',           // Column to sort by
  ...
  methods: {
    ...
    async renderUserProfilesTable() {
      ...
      let userProfilesTableData = [];
      for (let i = 0; i < this.userProfiles.length; i++) {
        userProfilesTableData.push({
          id: i,
          userProfileId: this.userProfiles[i].userProfileId,
          year: this.userProfiles[i].year,
          date: this.userProfiles[i].date,
          competitionId: this.userProfiles[i].competitionId,
          userProfileId: this.userProfiles[i].userProfileId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-user-profiles',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForUserProfilesChanged', this.searchQuery);
		//this.renderUserProfilesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalUserProfiles = false;

      const currentDate = new Date().getTime();
      this.userProfileToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.userProfileToAdd);
      console.log(jsonData);
      
      const res = await UserProfileService.addUserProfile(jsonData);
