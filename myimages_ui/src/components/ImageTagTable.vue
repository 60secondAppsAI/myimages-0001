        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalImageTags = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalImageTags">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ImageTag</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ImageTagId" type="text" placeholder="Enter ImageTagId" v-model="imageTagToAdd.imageTagId"></base-input>
  <base-input label="ImageId" type="text" placeholder="Enter ImageId" v-model="imageTagToAdd.imageId"></base-input>
  <base-input label="TagId" type="text" placeholder="Enter TagId" v-model="imageTagToAdd.tagId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="imageTags" :row-key="record => record.ImageTagId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ImageTagPictureView :imageTags="imageTags" />
          </div>
...
import ImageTagPictureView from './ImageTagPictureView.vue';


const imageTagsColumns = [
  "imageTagId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      modalImageTags: false,
        isTableView: true,

      columns: [
        {
          title: 'Image Tag Id',
		dataIndex: 'imageTagId',
          visible: true,
          scopedSlots: { customRender: 'imageTagId' },
          sorter: true
          //sorter: (a, b) => a.imageTagId - b.imageTagId,
          //sorter: (a, b) => a.imageTagId.localeCompare(b.imageTagId),
        },
        {
          title: 'Image Id',
		dataIndex: 'imageId',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} imageTags`,
      },
      ...
      sortBy: 'imageTagId',           // Column to sort by
  ...
  methods: {
    ...
    async renderImageTagsTable() {
      ...
      let imageTagsTableData = [];
      for (let i = 0; i < this.imageTags.length; i++) {
        imageTagsTableData.push({
          id: i,
          imageTagId: this.imageTags[i].imageTagId,
          year: this.imageTags[i].year,
          date: this.imageTags[i].date,
          competitionId: this.imageTags[i].competitionId,
          imageTagId: this.imageTags[i].imageTagId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-image-tags',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForImageTagsChanged', this.searchQuery);
		//this.renderImageTagsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalImageTags = false;

      const currentDate = new Date().getTime();
      this.imageTagToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.imageTagToAdd);
      console.log(jsonData);
      
      const res = await ImageTagService.addImageTag(jsonData);
