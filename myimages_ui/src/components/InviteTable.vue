        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalInvites = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInvites">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Invite</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="InviteId" type="text" placeholder="Enter InviteId" v-model="inviteToAdd.inviteId"></base-input>
  <base-input label="EventId" type="text" placeholder="Enter EventId" v-model="inviteToAdd.eventId"></base-input>
  <base-input label="UserProfileId" type="text" placeholder="Enter UserProfileId" v-model="inviteToAdd.userProfileId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="invites" :row-key="record => record.InviteId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InvitePictureView :invites="invites" />
          </div>
...
import InvitePictureView from './InvitePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'inviteId',
          visible: true,
          scopedSlots: { customRender: 'inviteId' },
          sorter: true
          //sorter: (a, b) => a.inviteId - b.inviteId,
          //sorter: (a, b) => a.inviteId.localeCompare(b.inviteId),
        },
        {
          title: 'Event Id',
		dataIndex: 'eventId',
      ...
      sortBy: 'inviteId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInvitesTable() {
      ...
      let invitesTableData = [];
      for (let i = 0; i < this.invites.length; i++) {
        invitesTableData.push({
          id: i,
          inviteId: this.invites[i].inviteId,
          year: this.invites[i].year,
          date: this.invites[i].date,
          competitionId: this.invites[i].competitionId,
          inviteId: this.invites[i].inviteId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-invites',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInvitesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalInvites = false;

      const currentDate = new Date().getTime();
      this.inviteToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.inviteToAdd);
      console.log(jsonData);
      
      const res = await InviteService.addInvite(jsonData);
