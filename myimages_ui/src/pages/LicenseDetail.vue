					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateLicense()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Licenses = true">Add</base-button>
								              <modal :show.sync="modal2Licenses">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">License Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="LicenseId" type="text" placeholder="Enter LicenseId" v-model="licenseDetails.licenseId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Owner Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="OwnerId" type="text" placeholder="Enter OwnerId" v-model="licenseDetails.ownerId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">License Type</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="LicenseType" type="text" placeholder="Enter LicenseType" v-model="licenseDetails.licenseType"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Valid Until</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="licenseDetails.validUntil"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateLicense() {
      ...
      const jsonData = JSON.stringify(this.licenseDetails);
      const res = await LicenseService.update(jsonData);
...
//        this.getAllLicenses();
    ...
    async getLicenseDetails() {
      try {
			let response = await LicenseService.get(this.licenseId);
			this.licenseDetails = response.data;
